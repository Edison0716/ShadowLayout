...
public class ShadowFrameLayout extends FrameLayout {
    ...
    private void initAttributes(AttributeSet attrs) {
        ...
        try {
            //默认是显示
            leftShow = attr.getBoolean(R.styleable.ShadowFrameLayout_hl_leftShow, true);
            rightShow = attr.getBoolean(R.styleable.ShadowFrameLayout_hl_rightShow, true);
            bottomShow = attr.getBoolean(R.styleable.ShadowFrameLayout_hl_bottomShow, true);
            topShow = attr.getBoolean(R.styleable.ShadowFrameLayout_hl_topShow, true);
            mCornerRadius = attr.getDimension(R.styleable.ShadowFrameLayout_hl_cornerRadius, getResources().getDimension(R.dimen.dp_0));
            //默认扩散区域宽度
            mShadowLimit = attr.getDimension(R.styleable.ShadowFrameLayout_hl_shadowLimit, getResources().getDimension(R.dimen.dp_5));

            //x轴偏移量
            mDx = attr.getDimension(R.styleable.ShadowFrameLayout_hl_dx, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.ShadowFrameLayout_hl_dy, 0);
            mShadowColor = attr.getColor(R.styleable.ShadowFrameLayout_hl_shadowColor, getResources().getColor(R.color.colorAccent));
            //判断传入的颜色值是否有透明度
            isAddAlpha(mShadowColor);
            mBackGroundColor = attr.getColor(R.styleable.ShadowFrameLayout_hl_shadowBackColor, getResources().getColor(R.color.colorAccent));
        } finally {
            attr.recycle();
        }
    }
